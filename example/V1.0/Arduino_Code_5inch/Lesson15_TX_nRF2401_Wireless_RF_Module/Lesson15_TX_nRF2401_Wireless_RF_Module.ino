/**
 * @file Lesson15_TX_nRF2401_Wireless_RF_Module.ino
...
/*---------------------------------------------------------------
 * Lesson15 Transmit N Rf2401 Wireless Rf Module Module
...
#error "Lesson15 requires PSRAM. In Arduino IDE select Tools > PSRAM > Enabled."
#endif
...
/**
 * @brief Initialize LVGL show counter label init and leave it ready for later operations.
 *
...
/**
 * @brief Run the ui counter task worker independently from the Arduino main loop.
 *
...
/**
 * @brief Run the nRF24L01 transmit task worker independently from the Arduino main loop.
 *
 * Called by the FreeRTOS scheduler after the task is created.
 *
 * @param param Value supplied by the caller for this operation.
...
bool display_touch_lvgl_init()
{
    ...
    if (!board->init()) {
        MAIN_ERROR("Panel configuration initialization failed");
        return false;
    }
...
#endif
    if (!board->begin()) {
        MAIN_ERROR("Display or touch hardware initialization failed");
        return false;
    }
    ...
    if (!lvgl_port_init(board->getLCD(), board->getTouch())) {
        MAIN_ERROR("LVGL port initialization failed");
        return false;
    }
    return true;
}
...
void setup() {
    ...
    if (psram_size == 0) {
        MAIN_ERROR("PSRAM is unavailable; select Tools > PSRAM > Enabled and upload again");
        while (true) {
            delay(1000);
        }
    }
    ...
    if (!display_touch_lvgl_init()) {
        MAIN_ERROR("Display startup stopped safely");
        while (true) {
            delay(1000);
        }
    }

    // nRF24L01 Wireless TX init (nRF24L01 receiver initialization)
    while (1) {
        uint8_t sw_level;
        ...
        if (SW_LEVEL_SELECT_TO_WIRELESS != sw_level)
        {
            ...
            // nRF24L01 wireless init
            esp_err_t err = nrf24_tx_init();  // Initialize nRF24L01 sender
            if (err != ESP_OK) {  // Check error
                MAIN_ERROR("nRF24L01 Wireless Module TX init failed");  // Halt if failed
                vTaskDelay(1000 / portTICK_PERIOD_MS);
            } else {
                MAIN_INFO("The nRF24L01 wireless module TX initialization was successful.");  // Log success
                break;
            }
        }
    }
