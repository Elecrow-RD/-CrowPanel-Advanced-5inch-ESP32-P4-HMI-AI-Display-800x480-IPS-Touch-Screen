...
esp_err_t BSP_SX1262::Sx1262_tx_init()
{
  ...
  int state = begin_sx1262_radio(bsp_sx_radio, 1.6);
...
esp_err_t BSP_NRF2401::NRF24_tx_init()
{
  ...
  nrf_hal.setSpiFrequency(1000000);
  nrf_hal.spiBegin();
...
/**
 * @brief Perform the nRF24L01 get transmit counter operation used by this lesson.
 *
 * Called from the application flow when this operation is required.
 *
 * @return Result produced by the operation; see the function body for its exact meaning.
...
/**
 * @brief Perform the nRF24L01 inc transmit counter operation used by this lesson.
 *
 * Called from the application flow when this operation is required.
 *
 * @return Nothing.
...
/**
 * @brief Handle the nRF24L01 set receive callback event and pass the result to the application.
 *
 * Called by the owning driver or framework when the event occurs.
 *
 * @param callback Value supplied by the caller for this operation.
