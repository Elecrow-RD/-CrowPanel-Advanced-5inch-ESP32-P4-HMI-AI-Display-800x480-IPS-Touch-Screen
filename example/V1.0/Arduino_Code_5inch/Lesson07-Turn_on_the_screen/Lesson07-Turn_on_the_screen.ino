...
void display_touch_lvgl_init() 
{
    ...
    const bool board_init_ok = board->init();
    Serial.printf("board->init(): %s\n", board_init_ok ? "OK" : "FAILED");
    assert(board_init_ok);
#if LVGL_PORT_AVOID_TEARING_MODE
    LCD *lcd = board->getLCD();
    // When avoid tearing function is enabled, the frame buffer number should be set in the board driver
    Serial.printf("Configuring %d RGB frame buffers...\n", LVGL_PORT_DISP_BUFFER_NUM);
    const bool frame_buffer_config_ok =
        (lcd != nullptr) && lcd->configFrameBufferNumber(LVGL_PORT_DISP_BUFFER_NUM);
    Serial.printf("RGB frame buffer configuration: %s\n", frame_buffer_config_ok ? "OK" : "FAILED");
    assert(frame_buffer_config_ok);
#endif
    Serial.println("Calling board->begin()...");
    const bool board_begin_ok = board->begin();
    Serial.printf("board->begin(): %s\n", board_begin_ok ? "OK" : "FAILED");
    assert(board_begin_ok);
    Serial.println("Display and Touch system online.");

    // The display library now owns initialization of the shared I2C host.
    // Turn the STC8-controlled backlight off only after that host is online.
