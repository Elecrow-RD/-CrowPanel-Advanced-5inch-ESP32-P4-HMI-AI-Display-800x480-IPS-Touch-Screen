...
void NoAudioCodec::SetOutputEnableHook(void (*hook)(bool enable, void* ctx), void* ctx) {
    std::lock_guard<std::mutex> lock(data_if_mutex_);
    output_enable_hook_ = hook;
    output_enable_hook_ctx_ = ctx;
}
...
void NoAudioCodec::EnableOutput(bool enable) {
    ...
    if (output_enable_hook_ != nullptr) {
        output_enable_hook_(enable, output_enable_hook_ctx_);
    }
}
