 ...
 * This file groups the lcd_display responsibilities so learners can
...
RgbLcdDisplay::RgbLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
                           ...
                           bool swap_bytes, bool buff_spiram, uint32_t buffer_size, bool double_buffer)
    : LcdDisplay(panel_io, panel, width, height) {
    ...
    ESP_LOGI(TAG, "Adding LCD display");
    if (buffer_size == 0) {
        buffer_size = static_cast<uint32_t>(width_) * static_cast<uint32_t>(height_);
    }
    const lvgl_port_display_cfg_t display_cfg = {
        .io_handle = panel_io_,
        .panel_handle = panel_,
...
MipiLcdDisplay::MipiLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    if (err == ESP_ERR_NOT_SUPPORTED) {
        // Some panels (like EK79007) don't implement disp_on_off, but they are already ON after init.
        ESP_LOGW(TAG, "esp_lcd_panel_disp_on_off not supported by this panel, skipping explicit display ON");
    } else {
        ESP_ERROR_CHECK(err);
    }
...
void LcdDisplay::SetPreviewImage(const lv_img_dsc_t* img_dsc) {
    DisplayLockGuard lock(this);
    if (content_ == nullptr) {
        ESP_LOGW(TAG, "SetPreviewImage: content_ is null, cannot show preview image");
        return;
    }
    
    ESP_LOGI(TAG, "SetPreviewImage called (img_dsc=%p, w=%lu, h=%lu)", 
             img_dsc, img_dsc ? (unsigned long)img_dsc->header.w : 0UL, img_dsc ? (unsigned long)img_dsc->header.h : 0UL);
    
    auto lvgl_theme = static_cast<LvglTheme*>(current_theme_);
    if (img_dsc != nullptr) {
        // Create a message bubble for image preview
        lv_obj_t* img_bubble = lv_obj_create(content_);
        if (img_bubble == nullptr) {
            ESP_LOGE(TAG, "Failed to create image bubble");
            return;
        }
        
        ESP_LOGI(TAG, "Created image bubble, setting properties...");
        ...
        lv_coord_t max_width = LV_HOR_RES * 40 / 100;  // 40% of screen width
        ...
        ESP_LOGI(TAG, "Image size: %ldx%ld, max: %ldx%ld", img_width, img_height, max_width, max_height);
        ...
        ESP_LOGI(TAG, "Calculated zoom: %ld (img: %ldx%ld -> max: %ldx%ld)", zoom, img_width, img_height, max_width, max_height);
        ...
        ESP_LOGI(TAG, "Preview image bubble created and displayed successfully (bubble size: %ldx%ld)", 
