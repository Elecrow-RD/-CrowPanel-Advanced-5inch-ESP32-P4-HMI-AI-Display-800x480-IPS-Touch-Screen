 ...
 * This file groups the p4_bsp_camera responsibilities so learners can
...
/**
 * @brief Continuously forward completed camera frames to the preview path.
 *
 * @param param Unused FreeRTOS task argument.
 * @return Nothing; the task runs for the lifetime of the camera object.
 *
 * Started by the constructor after the low-level camera pipeline is ready.
 */
...
/**
 * @brief Initialize the camera adapter and its preview task.
 *
 * @param None.
 * @return A camera object whose initialized_ flag reports startup success.
 *
 * Called once by ElecrowP4Board after enabling the camera's 3.3 V supply.
 */
...
/**
 * @brief Store the vision-service endpoint and authorization token.
 * @param url HTTP endpoint that accepts image explanation requests.
 * @param token Authorization token sent with the request.
 * @return Nothing.
 * @note Called when application configuration supplies a vision service.
 */
...
/**
 * @brief Capture a stable RGB565 frame for the LVGL preview.
 *
 * @param None.
 * @return true when a private preview buffer is delivered; false on a
 *         capture, allocation, or display-availability failure.
 *
 * Called when the application requests a new camera preview image.
 */
bool P4BspCamera::Capture() {
    ...
    // Copy into private PSRAM so the CSI controller can reuse its shared
    // capture buffer without changing pixels still owned by the display.
...
/**
 * @brief Request horizontal mirroring from the camera sensor.
 * @param enabled true to mirror the captured image.
 * @return true when the sensor accepts the setting.
 * @note Called when camera orientation settings change.
 */
...
/**
 * @brief Request vertical flipping from the camera sensor.
 * @param enabled true to flip the captured image.
 * @return true when the sensor accepts the setting.
 * @note Called when camera orientation settings change.
 */
...
/**
 * @brief Encode the current frame and request a visual explanation.
 *
 * @param question Natural-language question sent with the image.
 * @return Server response text when the HTTP request succeeds.
 *
 * Called by the application when a user asks the assistant to inspect the
 * camera view. Failures are reported through exceptions to the caller.
 */
