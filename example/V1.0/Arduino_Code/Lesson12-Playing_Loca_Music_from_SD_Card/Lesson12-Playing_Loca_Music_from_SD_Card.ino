 ...
 *
 * This file belongs to the Lesson12-Playing_Loca_Music_from_SD_Card course project. Comments explain the
 * teaching flow without changing the original program behavior.
 */

/*---------------------------------------------------------------
 * Lesson12-Playing Loca Music From Sd Card Module
...
esp_err_t sd_init()
{
    ...
    slot_config.width = 1;  // Use one-bit SDIO mode.
...
/**
 * @brief Perform the validate WAV header operation used by this lesson.
 *
 * Called from the application flow when this operation is required.
 *
 * @param file Value supplied by the caller for this operation.
 *
 * @return Result produced by the operation; see the function body for its exact meaning.
 */
...
/**
 * @brief Perform the audio play WAV SD operation used by this lesson.
 *
 * Called from the application flow when this operation is required.
 *
...
esp_err_t Audio_play_wav_sd(const char *filename)
{
    ...
    stc8_gpio_set_level(STC8_GPIO_OUT_AUDIO_SD, AUDIO_POWER_ENABLE);   // Enable audio power
    while (1)
    {
...
/**
 * @brief Initialize SD card init and leave it ready for later operations.
 *
 * Called during application startup before the feature is used.
 *
 * @return Nothing.
...
/**
 * @brief Initialize mic loudspeaker init and leave it ready for later operations.
 *
 * Called during application startup before the feature is used.
 *
 * @return Nothing.
