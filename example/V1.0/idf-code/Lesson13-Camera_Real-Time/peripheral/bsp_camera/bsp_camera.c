...
int camera_work()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        cam_buffer[i] = (uint8_t *)heap_caps_aligned_alloc(cache_line_size, 1024 * 600 * ((BITS_PER_PIXEL + 7) / 8), MALLOC_CAP_SPIRAM);
        cam_buffer_size[i] = 1024 * 600 * ((BITS_PER_PIXEL + 7) / 8);
    }
