...
static size_t camera_stride;
static size_t camera_frame_size;
...
int video_open()
{
...
#endif
    /* The ISP controller is attached to the CSI capture device.  /dev/video20
     * is metadata/statistics capture only and cannot provide image frames. */
    ...
    CAMERA_INFO("format: %u x %u", (unsigned)camera_format.fmt.pix.width,
                (unsigned)camera_format.fmt.pix.height);
    camera_video.camera_buf_hes = camera_format.fmt.pix.width;
    camera_video.camera_buf_ves = camera_format.fmt.pix.height;
    /* CSI RGB565 buffers are tightly packed in this driver.  Do not use the
     * unreliable bytesperline/sizeimage values returned by IDF 5.5.x here. */
    camera_stride = camera_video.camera_buf_hes * CAMERA_BYTES_PER_PIXEL;
    camera_frame_size = camera_stride * camera_video.camera_buf_ves;
...
{
    return camera_frame_size ? (uint32_t)camera_frame_size : camera_video.camera_buf_hes * camera_video.camera_buf_ves * 2;
}
...
static void camera_video_frame_operation(uint8_t *camera_buf, uint8_t camera_buf_index, uint32_t camera_buf_hes, uint32_t camera_buf_ves, size_t camera_buf_len)
{
    ...
    {
        const size_t src_stride = camera_stride ? camera_stride : camera_buf_hes * CAMERA_BYTES_PER_PIXEL;
        ...
        for (uint32_t y = 0; y < CAMERA_DISPLAY_HEIGHT; y++)
        {
            ...
            uint8_t *dst_row = display_buffer + y * dst_stride;
            /* IDF 5.5.x CSI delivers RGB565 high-byte first, while LVGL's
             * RGB565 buffer is little-endian.  Swap each pixel explicitly. */
            for (size_t x = 0; x < dst_stride; x += 2) {
...
int camera_work()
{
    ...
    if (lvgl_port_lock(0))
    {
        ...
        lv_canvas_set_buffer(camera_obj, display_buffer, CAMERA_DISPLAY_WIDTH, CAMERA_DISPLAY_HEIGHT, LV_COLOR_FORMAT_RGB565);
