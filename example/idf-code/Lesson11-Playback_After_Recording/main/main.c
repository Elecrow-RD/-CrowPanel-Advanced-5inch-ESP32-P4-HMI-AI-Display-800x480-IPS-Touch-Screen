...
void app_main(void)  // Main entry point for the application
{
	...
	err = audio_ctrl_init();  // Initialize audio amplifier and I2S playback control
	if (err != ESP_OK)
		init_or_halt("audio ctrl init", err);  // Check initialization result; halt if failed
	...
	if (err != ESP_OK)
		init_or_halt("audio init", err);  // Check initialization result; halt if failed
